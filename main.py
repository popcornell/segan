...
flags.DEFINE_integer("batch_size", 64, "Batch size (Def: 150).")
flags.DEFINE_integer("save_freq", 150, "Batch save freq (Def: 50).")
...
def main(_):
    ...
    for device in devices:
        if len(devices) > 1 and ('CPU' in device.name or  'XLA' in device.name): # DO NOT use XLA
